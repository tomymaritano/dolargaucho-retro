/**
 * BCRA Reservas Internacionales API
 *
 * Fetches international reserves data from Argentina.gob.ar API
 * Data represents BCRA's international reserves in millions of USD
 *
 * Data source: https://apis.datos.gob.ar/series/api/series/
 * Note: Series ID may need to be updated with actual BCRA reserves series
 */
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    // Try ArgentinaDatos API first (fallback to BCRA if needed)
    // Series ID for international reserves - using a common identifier
    ...
    if (!response.ok) {
      ...
      return res.status(200).json({
        ...
        latest: {
          ...
          value: 38928, // Mock value from research
          variation: null,
        },
        data: [],
        count: 0,
    ...
    // Cache for 12 hours (reserves update daily)
