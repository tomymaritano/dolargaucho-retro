/**
 * DolarTableHeader Component
 *
 * Single Responsibility: Render sortable table headers for DolaresTable
 * Extracted from DolaresTable.tsx (363 → 80 lines)
 */

import React from 'react';
import { FaSort, FaSortUp, FaSortDown } from 'react-icons/fa';
import { TableHeader, TableRow, TableHeaderCell } from '@/components/ui/Table';
import type { DolarSortField, SortDirection } from '@/hooks/useDolarSorting';

interface DolarTableHeaderProps {
  sortField: DolarSortField;
  sortDirection: SortDirection;
  onSort: (field: DolarSortField) => void;
}

interface SortIconProps {
  field: DolarSortField;
  sortField: DolarSortField;
  sortDirection: SortDirection;
}
...
export function DolarTableHeader({ sortField, sortDirection, onSort }: DolarTableHeaderProps) {
  return (
    <TableHeader>
      <TableRow hoverable={false}>
        ...
        {/* 7d % */}
        <TableHeaderCell align="right" sortable onSort={() => onSort('sparkline')}>
          <div className="flex items-center justify-end gap-2 group">
            7d %
