/**
 * UniversalLightweightChart Component
 *
 * Generic professional chart using TradingView Lightweight Charts
 * Can be used for any time-series data (FRED, ECB, Inflación, etc.)
 * Features:
...
interface UniversalLightweightChartProps {
  data: DataPoint[];
  title: string;
  color?: string;
...
export const UniversalLightweightChart = React.memo(function UniversalLightweightChart({
  ...
  isFavorite = false,
  onToggleFavorite,
}: UniversalLightweightChartProps) {
  ...
  const { chartData, latestValue, previousValue } = useMemo(() => {
    ...
    // Filter data by actual date range based on the latest data point (not "today")
    // This ensures we always show the requested timeframe relative to the most recent data
    const latestDataDate = new Date(data[data.length - 1].date);
    const cutoffDate = new Date(latestDataDate);
    cutoffDate.setMonth(latestDataDate.getMonth() - monthsToShow);

    const limitedData = data.filter((point) => {
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute top-2 left-2 right-2 z-10 flex items-start justify-between gap-2">
        ...
        <div className="bg-background/90 backdrop-blur-sm px-2 py-1.5 rounded-lg border border-border/5 pointer-events-none">
        ...
        <div className="flex items-start gap-1">
          {/* Period selector */}
          <div className="bg-background/90 backdrop-blur-sm rounded-lg border border-border/5 p-0.5 flex gap-0.5">
