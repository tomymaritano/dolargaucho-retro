/**
 * ECBLightweightChart Component
 *
 * Professional ECB exchange rate chart using TradingView Lightweight Charts
 * Features:
...
interface ECBLightweightChartProps {
  data: ECBDataPoint[];
  title: string;
  color?: string;
...
export const ECBLightweightChart = React.memo(function ECBLightweightChart({
  ...
  const { chartData, latestValue, previousValue } = useMemo(() => {
    ...
    const latestValue = limitedData[limitedData.length - 1]?.rate || 0;
    const previousValue = limitedData[limitedData.length - 2]?.rate || 0;
