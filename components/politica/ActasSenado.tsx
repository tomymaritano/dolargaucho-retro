...
import { useActasSenado, useActasSenadoByYear } from '@/hooks/usePolitica';
import { FaFileAlt, FaExternalLinkAlt, FaSearch, FaCalendarAlt, FaSpinner } from 'react-icons/fa';

interface ActasSenadoProps {
  limit?: number;
}
...
export const ActasSenado = React.memo(function ActasSenado({ limit = 50 }: ActasSenadoProps) {
  ...
  const { data: todasActas, isLoading: loadingTodas } = useActasSenado();
  const { data: actasPorAño, isLoading: loadingPorAño } = useActasSenadoByYear(
            ...
            <div className="space-y-3">
              {actasFiltradas.map((acta, index) => (
                <div
                  ...
                  className="p-4 glass border border-border rounded-lg hover:bg-panel/10 transition-colors"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <h4 className="text-white font-semibold mb-1">
                        {acta.orden_dia || 'Sesión del Senado'}
                      </h4>
